...
intervaloDeTempo = list(map(int, input().split()))
tempoEvelovidade = []
distanciaTotal = []

for dias in range(intervaloDeTempo[0],):
    horas = list(map(int, input().split()))
    tempoEvelovidade.append(horas)
    
for numeros in tempoEvelovidade:
    calculo = numeros[0] * numeros[1]
    distanciaTotal.append(calculo)

print(sum(distanciaTotal))
