...
def busca_binaria(inicio, fim, numeroProcurado):
    ...
    while numeroProcurado != meio:
        
        if numeroProcurado == meio:
            resposta = f'Você achou o número {numeroProcurado}'
            return resposta

        if numeroProcurado > meio:
            busca_binaria(meio + 1, fim, numeroProcurado)

        if numeroProcurado < meio:
            busca_binaria(inicio, meio - 1, numeroProcurado)
