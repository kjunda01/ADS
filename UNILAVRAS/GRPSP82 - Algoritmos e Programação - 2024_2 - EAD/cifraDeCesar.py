mensagem = 'Lorem, ipsum dolor sit amet consectetur adipisicing elit. Atque, eveniet vitae? Labore autem sit, quod, corrupti dolore atque unde voluptate eos cupiditate cum excepturi accusamus dolorum itaque molestiae pariatur veniam!'
...
def cesar(msgBruta):
    ...
    msgCodificada = msgParaCodificar

    return msgCodificada


def descriptografarCesar(msgCodificada):
    mensagemDecodificar = ''
    for caracter in range(len(msgCodificada)):
        mensagemDecodificar += mensagemDecodificar.join(chr(ord(msgCodificada[caracter]) - 3))

    msgDecodificada = mensagemDecodificar

    return msgDecodificada


mostrarCriptografia = cesar(mensagem)
mostrarDescriptografia = descriptografarCesar(cesar(mensagem))

print(f"Mensagem criptografada: {mostrarCriptografia}\n")
print(f"Mensagem Descriptografada: {mostrarDescriptografia}")
